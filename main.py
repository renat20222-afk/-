...
def get_welcome_text(user_name):
    return (
        ...
        f"Здесь собрано всё самое необходимое для звука:\n\n"
        f"🎹 <b>Драм киты</b> | 🔌 <b>Плагины</b> | 🎚 <b>DAW</b>\n\n"
        f"Выбирай нужную категорию ниже! 👇"
    )
...
def init_db():
    ...
    cursor = conn.cursor()
    # Таблица товаров (добавили subcategory)
    ...
    # Автоматическое добавление колонки subcategory в старую базу, если её не было
    try:
...
class AdminStates(StatesGroup):
    waiting_for_subcategory = State() # Выбор жанра для драмкитов
...
async def start_cmd(message: types.Message):
    ...
    try:
        await message.answer_photo(FSInputFile(START_PHOTO_PATH), caption=text, parse_mode="HTML", reply_markup=main_menu())
    except:
...
async def back_to_menu(callback: types.CallbackQuery):
    ...
    try:
        await callback.message.answer_photo(FSInputFile(START_PHOTO_PATH), caption=text, parse_mode="HTML", reply_markup=main_menu())
    except:
        await callback.message.answer(text, parse_mode="HTML", reply_markup=main_menu())

# Меню выбора жанров для драм китов
...
async def show_list(callback: types.CallbackQuery):
    ...
    # Проверяем, это обычный список (3 элемента) или с разделением по жанрам (4 элемента)
    if len(data) == 4:
    ...
    for i_id, name in items[start:end]:
        # Передаем ID элемента и страницу
...
async def view_item(callback: types.CallbackQuery):
    ...
    caption = f"<b>{name}</b>\n\n{desc or ''}"
    builder = InlineKeyboardBuilder()
    if f_id: builder.row(types.InlineKeyboardButton(text="📥 Скачать файл", callback_data=f"dl_{item_id}"))
...
# --- АДМИН-ФУНКЦИИ (УДАЛЕНИЕ И ИЗМЕНЕНИЕ) ---
...
async def post_command(message: types.Message, state: FSMContext):
    ...
    await message.answer("📝 Отправь мне сообщение, которое нужно разослать всем (это может быть текст, фото с описанием, документ или видео):")
...
async def perform_post(message: types.Message, state: FSMContext):
    ...
    await status_msg.edit_text(
        ...
        f"❌ Не доставлено (заблокировали бота): {blocked_count}",
        parse_mode="HTML"
    )

# --- АДМИН-ПАНЕЛЬ (ДОБАВЛЕНИЕ) ---
...
async def admin_add(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete()
    if i_type == "drumkit":
        # Предлагаем выбрать жанр драм кита
        builder = InlineKeyboardBuilder()
        for key, name in DRUMKIT_GENRES.items():
            builder.row(types.InlineKeyboardButton(text=name, callback_data=f"admin_genre_{key}"))
        await callback.message.answer("Выберите жанр драм-кита:", reply_markup=builder.as_markup())
        await state.set_state(AdminStates.waiting_for_subcategory)
    else:
        await state.update_data(subcategory=None)
        await callback.message.answer(f"Добавление ({i_type}). Введите название:")
        await state.set_state(AdminStates.waiting_for_name)
...
async def admin_select_genre(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.delete()
    await callback.message.answer(f"Добавление Драм кита ({DRUMKIT_GENRES[genre]}). Введите название:")
    await state.set_state(AdminStates.waiting_for_name)

@dp.message(AdminStates.waiting_for_name)
async def adm_name(message: types.Message, state: FSMContext):
    await state.update_data(name=message.text)
    await message.answer("Отправь ФАЙЛ или ССЫЛКУ на Google Drive (или /skip):")
...
async def adm_google(message: types.Message, state: FSMContext):
    if message.document:
        ...
        await state.update_data(link_google=None if message.text == "/skip" else message.text, file_id=None)
    await message.answer("Ссылку на Яндекс Диск (или /skip):")
...
async def adm_yandex(message: types.Message, state: FSMContext):
    await state.update_data(link_yandex=None if message.text == "/skip" else message.text)
...
async def adm_photo(message: types.Message, state: FSMContext):
    ...
    if data.get('edit_id'):
        update_item_in_db(data['edit_id'], data['name'], data['link_google'], data['link_yandex'], data['file_id'], data['desc'], photo_id, data.get('subcategory'))
        await message.answer("✅ Успешно обновлено!")
    else:
        add_item_to_db(data['name'], data['link_google'], data['link_yandex'], data['file_id'], data['item_type'], data['desc'], photo_id, data.get('subcategory'))
