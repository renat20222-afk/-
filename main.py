...
# Узнать можно через @getmyid_bot
...
class MaintenanceAndSubMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        ...
        # Разрешаем команды старта и смены языка
        is_basic = False
...
@dp.message(Command("start"), Command("eng"))
...
async def verify_sub(callback: types.CallbackQuery):
    ...
    if await check_subscription(callback.bot, callback.from_user.id):
        await callback.message.delete()
        text = TEXTS[lang]["welcome"].format(name=callback.from_user.first_name)
        try: await callback.message.answer_photo(FSInputFile(START_PHOTO_PATH), caption=text, parse_mode="HTML", reply_markup=main_menu_kb(lang))
        except: await callback.message.answer(text, parse_mode="HTML", reply_markup=main_menu_kb(lang))
    else:
