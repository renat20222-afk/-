...
async def send_welcome_msg(bot: Bot, user_id: int, text: str, kb=None, photo_path=None, user_msg_id=None):
    # Удаляем текстовую команду пользователя (/start или /eng) для чистоты
    if user_msg_id:
        try: await bot.delete_message(user_id, user_msg_id)
        except: pass
        
    # Удаляем абсолютно все старые сообщения бота, чтобы воссоздать приветствие с нуля
    ...
    # Отправляем главное приветствие
...
async def send_sub_msg(bot: Bot, user_id: int, text: str, kb=None, photo_id=None):
    # Удаляем только старое сообщение подраздела, приветствие сверху не трогаем!
...
async def show_category(c: types.CallbackQuery):
    ...
    # Исправлено: выбираем правильный текст вопроса в зависимости от категории (drumkit или daw)
