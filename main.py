...
# ЦИФРОВОЙ ID КАНАЛА (начинается на -100)
...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS users 
                      (user_id INTEGER PRIMARY KEY, lang TEXT DEFAULT NULL, last_msg_id INTEGER DEFAULT NULL)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)''')
    cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('bot_enabled', '1')")
    
    # Миграция колонок (на случай старой БД)
    try: cursor.execute("ALTER TABLE users ADD COLUMN last_msg_id INTEGER DEFAULT NULL")
    except: pass
    try: cursor.execute("ALTER TABLE users ADD COLUMN lang TEXT DEFAULT NULL")
    except: pass
    try: cursor.execute("ALTER TABLE items ADD COLUMN subcategory TEXT DEFAULT NULL")
    except: pass
...
# --- ФУНКЦИЯ УДАЛЕНИЯ СТАРОГО И ОТПРАВКИ НОВОГО СООБЩЕНИЯ ---
async def clear_and_send(bot: Bot, user_id: int, text: str, reply_markup=None, photo=None, parse_mode="HTML"):
    conn = sqlite3.connect('assets.db')
    cursor = conn.cursor()
    ...
    if row and row[0]:
        try:
            await bot.delete_message(chat_id=user_id, message_id=row[0])
        except Exception:
            pass
    ...
    if photo:
        try:
            new_msg = await bot.send_photo(chat_id=user_id, photo=photo, caption=text, reply_markup=reply_markup, parse_mode=parse_mode)
    ...
    cursor.execute("INSERT INTO users (user_id, last_msg_id) VALUES (?, ?) ON CONFLICT(user_id) DO UPDATE SET last_msg_id=?", (user_id, new_msg.message_id, new_msg.message_id))
    conn.commit()
    conn.close()
    return new_msg
...
# --- MIDDLEWARE ДЛЯ СООБЩЕНИЙ И КНОПОК ---
class MaintenanceAndSubMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        ...
        # Разрешаем базовые команды без проверок
        is_basic = False
        if isinstance(event, types.Message) and event.text:
            is_basic = event.text.startswith("/start") or event.text.startswith("/eng")
        
        is_cb = isinstance(event, types.CallbackQuery) and (event.data.startswith("setlang_") or event.data == "check_sub")

        if is_basic or is_cb: 
            return await handler(event, data)
...
dp.message.outer_middleware(MaintenanceAndSubMiddleware())
dp.callback_query.outer_middleware(MaintenanceAndSubMiddleware())
...
async def cmd_start_lang(message: types.Message):
    try: await message.delete()  # Удаляем команду пользователя
    except: pass
...
async def cmd_menu(message: types.Message):
    ...
    text = TEXTS[lang]["welcome"].format(name=safe_name)
...
async def drumkit_genres_cb(callback: types.CallbackQuery):
    # Плавное редактирование существующего сообщения
    lang = get_user_lang(callback.from_user.id) or "ru"
    kb = InlineKeyboardBuilder()
    ...
    if callback.message.photo:
        await callback.message.edit_caption(caption=TEXTS[lang]["select_genre"], reply_markup=kb.as_markup())
    else:
        await callback.message.edit_text(TEXTS[lang]["select_genre"], reply_markup=kb.as_markup())
    await callback.answer()
...
async def list_items_cb(callback: types.CallbackQuery):
    ...
    if not items: 
        return await callback.answer(TEXTS[lang]["empty"], show_alert=True)
    ...
    # Смена контента: используем clear_and_send
    await clear_and_send(
        bot=callback.bot,
        user_id=callback.from_user.id,
        text=txt,
...
async def dl_cb(callback: types.CallbackQuery):
    # Файлы шлем просто в чат, меню при этом не удаляем!
...
async def turn_off(m: types.Message):
    if m.from_user.id == ADMIN_ID: set_bot_enabled(False); await m.answer("🔴 Бот временно отключен.")

@dp.message(Command("on"))
async def turn_on(m: types.Message):
    if m.from_user.id == ADMIN_ID: set_bot_enabled(True); await m.answer("🟢 Бот успешно включен.")
