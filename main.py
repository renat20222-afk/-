...
# Цифровой ID вашего канала (начинается на -100)
# Бот ОБЯЗАТЕЛЬНО должен быть добавлен в этот канал как АДМИНИСТРАТОР!
CHANNEL_ID = -1001552554703 

# Путь к фото приветствия
START_PHOTO_PATH = 'welcome.jpg'

# Глобальный кэш для мгновенного удаления (чтобы избежать дублирования при спаме)
user_last_message = {}
...
async def is_subscribed(bot: Bot, user_id: int) -> bool:
    try:
        ...
        # чтобы бот требовал настроить его в канале, а не пускал всех подряд без проверки
        return False 

# --- СИСТЕМА ОДНОГО ОКНА (УДАЛЕНИЕ И ОТПРАВКА СООБЩЕНИЙ) ---
async def send_interface(bot: Bot, user_id: int, text: str, kb=None, photo=None):
    # Достаем ID старого сообщения бота из памяти или БД
    ...
    try:
        ...
        logging.error(f"Ошибка отправки интерфейса: {e}")

# --- MIDDLEWARE ДЛЯ АВТО-ОЧИСТКИ И ПРОВЕРОК ---
class MainMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        user = event.from_user
        if not user: return await handler(event, data)

        # Удаляем входящее текстовое сообщение пользователя, чтобы чат был чистым
        if isinstance(event, types.Message):
            try: await event.delete()
            except: pass

        # Админ-команды пропускаем всегда без ограничений тех. обслуживания
        if isinstance(event, types.Message) and event.text and event.text.startswith(("/admin", "/on", "/off")):
            return await handler(event, data)

        # 1. Проверка технического обслуживания
        ...
        if status and status[0] == '0' and user.id != ADMIN_ID:
            if isinstance(event, types.Message):
                await event.answer(TEXTS["ru"]["maintenance"])
            else:
                await event.answer(TEXTS["ru"]["maintenance"], show_alert=True)
            return

        # 2. Пропускаем команды старта и выбора языка в обход языкового барьера
        ...
        # Пропускаем только кнопку подтверждения подписки "check_sub", чтобы хендлер мог ее обработать
...
async def cmd_start(message: types.Message):
    ...
    if lang:
        # Если язык выбран, проверяем подписку перед показом меню
        ...
        # Если не выбран — просим выбрать язык
        kb = InlineKeyboardBuilder()
        kb.row(types.InlineKeyboardButton(text="🇷🇺 Русский", callback_data="setlang_ru"),
               types.InlineKeyboardButton(text="🇬🇧 English", callback_data="setlang_en"))
...
async def set_lang(callback: types.CallbackQuery):
    ...
    # Сразу после выбора языка строго проверяем подписку!
...
async def view_item(callback: types.CallbackQuery):
    ...
    if callback.from_user.id == ADMIN_ID:
        kb.row(types.InlineKeyboardButton(text="🗑 Удалить (Admin)", callback_data=f"del_{i_id}_{pg}"))
...
async def adm_panel(m: types.Message):
    ...
    await m.answer("Панель управления администратора:", reply_markup=kb.as_markup())
...
async def add_start(c: types.CallbackQuery, state: FSMContext):
    ...
    if t == "drumkit":
        ...
        await c.message.answer("Выберите жанр драм-кита:", reply_markup=kb.as_markup())
    else:
...
async def add_genre(c: types.CallbackQuery, state: FSMContext):
    ...
    await c.message.answer("Введите название драм-кита:")
    await state.set_state(AdminStates.wait_name)

@dp.message(AdminStates.wait_name)
async def adm_name(m: types.Message, state: FSMContext):
    await state.update_data(name=m.text)
    await m.answer("Отправьте файл в чат (как документ) или введите /skip:")
...
async def adm_file(m: types.Message, state: FSMContext):
    ...
    await m.answer("Укажите ссылку Google Drive или введите /skip:")
...
async def adm_google(m: types.Message, state: FSMContext):
    ...
    await m.answer("Укажите ссылку Yandex Disk или введите /skip:")
...
async def adm_yandex(m: types.Message, state: FSMContext):
    ...
    await m.answer("Введите подробное описание:")
...
async def adm_desc(m: types.Message, state: FSMContext):
    ...
    await m.answer("Отправьте превью-фотографию или введите /skip:")
...
async def adm_photo(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ Элемент успешно добавлен в базу данных бота!")
    await state.clear()

@dp.callback_query(F.data.startswith("del_"))
async def adm_del(c: types.CallbackQuery):
    _, i_id, pg = c.data.split("_")
    db_query("DELETE FROM items WHERE id = ?", (i_id,))
    await c.answer("Успешно удалено!", show_alert=True)
    # Возвращаем администратора назад в меню
    row = db_query("SELECT lang FROM users WHERE user_id = ?", (c.from_user.id,), fetchone=True)
    await show_menu(c.from_user.id, row[0] if row else "ru")

# --- СТАРТ РАБОТЫ ---
async def main():
    init_db()
    await bot.set_my_commands([
        types.BotCommand(command="start", description="Запуск бота / Заново"),
        types.BotCommand(command="menu", description="Главное меню"),
        types.BotCommand(command="eng", description="Change language / Сменить язык"),
        types.BotCommand(command="admin", description="Админ панель (Для администратора)")
    ])
