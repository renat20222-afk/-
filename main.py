...
# Цифровой ID вашего канала (обязательно начинается на -100)
...
async def is_subscribed(bot: Bot, user_id: int) -> bool:
    # ЕСЛИ ХОТИТЕ ПРОВЕРИТЬ СЕБЯ (АДМИНИСТРАТОРА), ЗАКОММЕНТИРУЙТЕ СЛЕДУЮЩУЮ СТРОКУ:
    ...
    try:
        ...
        # Если бот не админ в канале, он физически не может проверить, поэтому пропускаем
        return True 
...
async def send_interface(bot: Bot, user_id: int, text: str, kb=None, photo=None):
    # Пытаемся достать ID старого сообщения бота из кэша памяти или БД
    ...
    try:
        ...
        # Обновляем ID последнего сообщения в кэше и базе данных
...
class MainMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        ...
        # Удаляем входящее сообщение пользователя (команду), чтобы очистить чат
        ...
        # 2. Пропускаем команды старта и выбора языка в обход других проверок
        ...
        # 4. Проверка обязательной подписки
...
async def cmd_start(message: types.Message):
    ...
    if lang:
        # Если язык уже выбран, сразу открываем главное меню
        await show_menu(message.from_user.id, lang)
    else:
        # Если не выбран — просим выбрать
        kb = InlineKeyboardBuilder()
        kb.row(types.InlineKeyboardButton(text="🇷🇺 Русский", callback_data="setlang_ru"),
               types.InlineKeyboardButton(text="🇬🇧 English", callback_data="setlang_en"))
...
async def cmd_eng(message: types.Message):
    # Команда /eng принудительно вызывает окно выбора языка
    kb = InlineKeyboardBuilder()
    kb.row(types.InlineKeyboardButton(text="🇷🇺 Русский", callback_data="setlang_ru"),
           types.InlineKeyboardButton(text="🇬🇧 English", callback_data="setlang_en"))
    await send_interface(message.bot, message.from_user.id, TEXTS["ru"]["select_lang"], kb.as_markup())
