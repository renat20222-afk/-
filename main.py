...
CHANNEL_USERNAME = '@soundroid'  # Используем юзернейм для надежности
...
async def is_subscribed(bot: Bot, user_id: int) -> bool:
    # Для теста подписки на личном аккаунте можете закомментировать строку ниже:
...
class MainMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        ...
        if status and status[0] == '0':
            await (event.answer(TEXTS["ru"]["maintenance"]) if isinstance(event, types.Message) else event.answer(TEXTS["ru"]["maintenance"], show_alert=True))
            return
...
async def main():
    ...
    # Меню команд для ВСЕХ пользователей
    ...
    # Меню команд ТОЛЬКО для администратора
    try:
        await bot.set_my_commands(
            commands=[
                types.BotCommand(command="start", description="Запуск бота"),
                ...
                types.BotCommand(command="off", description="Отключить бота (Тех. работы)")
            ],
            scope=types.BotCommandScopeChat(chat_id=ADMIN_ID)
        )
    except Exception as e:
        logging.error(f"Не удалось установить админские команды в меню: {e}")
